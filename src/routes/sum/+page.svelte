	...
	onMount(async () => {
		if (form?.schema && form?.data) {
			...
			columns = form.schema.fields.map((field, index) => ({
				...
				sortable: true,
				filterValue:
					field.type === 'string' ? (v) => v[field.name].charAt(0).toLowerCase() : undefined
			}));
		}
	});
</script>
...
<div class="flex flex-col justify-center items-center h-full w-full">
	<div class="card w-96 bg-base-300 shadow-xl">
		<div class="card-body">
			...
			<form method="POST" enctype="multipart/form-data">
				...
				<div class="card-actions {form ? 'justify-center' : 'justify-end'} mt-4">
					{#if !form && spinner}
						<Spinner />
					{/if}

					{#if !form}
						<button class="btn btn-primary" disabled={!$sumForm.valid} on:click={showSpinner}>
							Submit
						</button>
					{/if}

					<button class="btn btn-secondary" formaction="" on:click|preventDefault={reset}
						>Reset form</button
					>
				</div>
			</form>
	...
	<div class="overflow-x-auto justify-center align-middle items-center w-full p-10">
